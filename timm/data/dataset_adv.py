...
class Dataset_adv(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        path_list = path.split('/') # img path
        folder_name = path_list[-2] # folder name
        dir_name = os.path.join(self.root[:self.root.find('/')], 'train_ns') # imagenet/train_ns
        filename = folder_name.pop(-1) # filename.jpg
        raw_name = filename[:filename.find('.')] # filename
        folder_dir = os.path.join(dir_name, folder_name) # imagenet/train_ns/folder
        out_dir = os.path.join(folder_dir, 'out') # imagenet/train_ns/folder/out
        traj_dir = os.path.join(folder_dir, 'traj') # imagenet_train_ns/folder/ns
        out = np.load(os.path.join(out_dir, raw_name+'.npy')) # out.npy
        traj_list = []
        for i in range(8):
            traj_list.append(np.load(os.path.join(traj_dir, raw_name+'_{}.npy'.format(i))))
        return img, target, out, traj_list
